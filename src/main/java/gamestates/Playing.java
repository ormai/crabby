...
public class Playing extends State implements StateMethods {
    ...
    private final BufferedImage[] shipImages;
    private final ArrayList<DialogueEffect> dialogEffects = new ArrayList<>();
    ...
    private BufferedImage[] questionImages, exclamationImages;
    ...
    // creating the level from lvlImages. Just like any other object.
    ...
    public Playing(Game game) {
        ...
        for (int i = 0; i < shipImages.length; i++) {
            shipImages[i] = temp.getSubimage(i * 78, 0, 78, 72);
        }
    ...
    private void loadDialogue() {
        ...
        // This is a simple way of avoiding ConcurrentModificationException error. Adding to a
        // list that is being looped through.
    ...
    private void loadDialogueImages() {
        BufferedImage temp = LoadSave.GetSpriteAtlas(LoadSave.QUESTION_ATLAS);
        questionImages = new BufferedImage[5];
        for (int i = 0; i < questionImages.length; i++) {
            questionImages[i] = temp.getSubimage(i * 14, 0, 14, 12);
        }

        BufferedImage temp2 = LoadSave.GetSpriteAtlas(LoadSave.EXCLAMATION_ATLAS);
        exclamationImages = new BufferedImage[5];
        for (int i = 0; i < exclamationImages.length; i++) {
            exclamationImages[i] = temp2.getSubimage(i * 14, 0, 14, 12);
        }
    }
    ...
    private void updateShipAni() {
        ...
        float shipHeightChange = 0.05f * Game.SCALE;
        ...
        for (DialogueEffect de : dialogEffects) {
            if (de.isActive()) {
                if (de.getType() == QUESTION) {
                    g.drawImage(questionImages[de.getAniIndex()], de.getX() - xLvlOffset, de.getY()
                            , DIALOGUE_WIDTH, DIALOGUE_HEIGHT, null);
                } else {
                    g.drawImage(exclamationImages[de.getAniIndex()], de.getX() - xLvlOffset,
                            de.getY(), DIALOGUE_WIDTH, DIALOGUE_HEIGHT, null);
                }
            }
        }
    }
    ...
    public void draw(Graphics g) {
        ...
        if (drawShip) {
            g.drawImage(shipImages[shipAni], (int) (100 * Game.SCALE) - xLvlOffset,
                    (int) ((288 * Game.SCALE) + shipHeightDelta), (int) (78 * Game.SCALE),
                    (int) (72 * Game.SCALE), null);
        }
    ...
    private void drawClouds(Graphics g) {
        for (int i = 0; i < 4; i++) {
            g.drawImage(bigCloud, i * BIG_CLOUD_WIDTH - (int) (xLvlOffset * 0.3),
                    (int) (204 * Game.SCALE), BIG_CLOUD_WIDTH, BIG_CLOUD_HEIGHT, null);
        }
